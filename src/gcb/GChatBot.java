...
public class GChatBot implements GarenaListener {
	...
	public String command(MemberInfo member, String command, String payload) {
		...
		if(muteList.contains(member.username)) {
			chatthread.queueChat(garena.id, "You are muted! An examiner must unmute you to allow you to use commands again", member.userID);
			return null;
		}
		...
		if(memberRank >= LEVEL_ROOT_ADMIN) {
			...
			if(command.equals("exit")) {
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "addadmin <username>. For further help use " + trigger + "help addadmin", member.userID);
					return null;
				}
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "deleterank <username>. For further help use " + trigger + "help deleterank", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				UserInfo targetUser = userFromName(target.toLowerCase());
				...
				if(!payload.equals("")) {
					if(!GarenaEncrypt.isInteger(parts[0])) {
						chatthread.queueChat(garena.id, invalidFormat, member.userID);
						return null;
					}
					if(!validIP(parts[1])) {
						chatthread.queueChat(garena.id, invalidFormat, member.userID);
		...
		if(memberRank >= LEVEL_ADMIN) {
			...
			if(command.equals("addexaminer")) {
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "addadmin <username>. For further help use " + trigger + "help addexaminer", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "addadmin <username>. For further help use " + trigger + "help addvip", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				if(target.equalsIgnoreCase(member.username)) {
				...
				if(parts.length < 3 || !GarenaEncrypt.isInteger(parts[1])) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "ban <username> <length_in_hours> <reason>. For further help use " + trigger + "help ban", member.userID);
					return null;
				}
				...
				if(sqlthread.ban(target, ipAddress, currentDate, member.username, reason, expireDate)) {
					if(roomBan && channelAdmin) {
						chatthread.queueChat(garena.id, "Success! " + target + " can no longer access this room. For information about this ban use " + trigger + "baninfo " + target, ANNOUNCEMENT);
						try {
							Thread.sleep(1000);
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "unban <username>. For further help use " + trigger + "help unban", member.userID);
					return null;
				}
				if(!sqlthread.doesBanExist(payload.toLowerCase())) {
					chatthread.queueChat(garena.id, "Failed. " + payload + " was not banned by this bot!", ANNOUNCEMENT);
					return null;
				}
				...
				if(sqlthread.unban(payload.toLowerCase()) && sqlthread.setUnbannedBy(payload, member.username)) {
					if(roomUnban && channelAdmin) {
						chatthread.queueChat(garena.id, "Success! " + payload + " is no longer banned from this room and GCB", ANNOUNCEMENT);
				...
				String[] parts = payload.split(" ", 2);
				if(parts.length < 2) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "kick <username> <reason>. For further help use " + trigger + "help kick", member.userID);
					return null;
				}
				...
				String[] parts = payload.split(" ", 2);
				if(parts.length < 2) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "quickkick <username> <reason>. For further help use " + trigger + "help quickkick", member.userID);
					return null;
				}
				String target = trimUsername(parts[0]);
				if(target.equalsIgnoreCase(member.username)) {
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "addautoannounce <message>. For further help use " + trigger + "help addautoannounce", member.userID);
					return null;
				}
				...
				if(payload.equals("") || !GarenaEncrypt.isInteger(payload)) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "setautoannounceinterval <time_in_seconds>. For further help use " + trigger + "help setautoannounceinterval", member.userID);
					return null;
				}
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "demote <username>. For further help use " + trigger + "help demote", member.userID);
				}
				String target = trimUsername(removeSpaces(payload));
				if(target.equalsIgnoreCase(member.username)) {
					return "Failed. You can not demote yourself!";
				...
				if(targetUser.rank == LEVEL_ROOT_ADMIN) {
					chatthread.queueChat(garena.id, "Failed. " + target + " is a Root Admin!", MAIN_CHAT);
					return null;
				} else if(targetUser.rank == LEVEL_ADMIN) {
					if(memberRank == LEVEL_ROOT_ADMIN) {
						if(sqlthread.setRank(target.toLowerCase(), member.username, LEVEL_EXAMINER)) {
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "banword <word>. For further help use " + trigger + "help banword", member.userID);
					return null;
				}
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "unbanword <word>. For further help use " + trigger + "help unbanword", member.userID);
					return null;
				}
				...
				if(parts.length < 3 || GarenaEncrypt.isInteger(parts[1])) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "multiban <username1>,<username2>,<username_X> <length_in_hours> <reason>. For further help use " + trigger + "help multiban", member.userID);
				}
				int banLength = Integer.parseInt(parts[1]);
				String reason = parts[2];
				String currentDate = time();
				...
				for(int i = 0; i < usernames.length; i++) {
					...
					if(sqlthread.ban(usernames[i], ipAddress, currentDate, member.username, reason, expireDate)) {
						if(roomBan && channelAdmin) {
							chatthread.queueChat(garena.id, "Success! " + usernames[i] + " can no longer access this room. For information about this ban use " + trigger + "baninfo " + usernames[i], ANNOUNCEMENT);
							try {
								Thread.sleep(1000);
							} catch(InterruptedException e) {
								if(Main.DEBUG) {
		...
		if(memberRank >= LEVEL_EXAMINER) {
			...
			if(command.equals("announce")) {
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "announce <message>. For further help use " + trigger + "help announce", member.userID);
					return null;
				}
				chatthread.queueChat(garena.id, payload, ANNOUNCEMENT);
				return null;
			} else if(command.equals("say")) {
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "say <message>. For further help use " + trigger + "help say", member.userID);
					return null;
				}
				chatthread.queueChat(garena.id, payload, MAIN_CHAT);
				return null;
			} else if(command.equals("whisper")) {
				String[] parts = payload.split(" ", 2);
				if(parts.length < 2) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "whisper <username> <message>. For further help use " + trigger + "help whisper", member.userID);
					return null;
				}
				...
				chatthread.queueChat(garena.id, message, target.userID);
				return null;
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "mute <username>. For further help use " + trigger + "help mute", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				if(target.equalsIgnoreCase(member.username)) {
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "unmute <username>. For further help use " + trigger + "help unmute", member.userID);
					return null;
				}
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "traceuser <username>. For further help use " + trigger + "help traceuser", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				UserInfo targetUser = userFromName(target.toLowerCase());
				...
				chatthread.queueChat(garena.id, "http://www.dnsstuff.com/tools/whois/?ip=" + targetUser.ipAddress + " or http://www.ip-adress.com/ip_tracer/" + targetUser.ipAddress, member.userID);
				return null;
				...
				chatthread.queueChat(garena.id, "http://www.dnsstuff.com/tools/whois/?ip=" + payload + " or http://www.ip-adress.com/ip_tracer/" + payload, member.userID);
				return null;
			} else if(command.equals("checkuserip")) {
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "checkuserip <username>. For further help use " + trigger + "help checkuserip", member.userID);
					return null;
				}
				payload = removeSpaces(trimUsername(payload));
		...
		if(memberRank >= LEVEL_VIP) {
			...
			if(command.equals("addsafelist")) {
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "addadmin <username>. For further help use " + trigger + "help addsafelist", member.userID);
					return null;
				}
				String target = trimUsername(removeSpaces(payload));
				if(target.equalsIgnoreCase(member.username)) {
				...
				String target = trimUsername(removeSpaces(payload));
				if(target.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "getpromote <username>. For further help use " + trigger + "help getpromote", member.userID);
					return null;
				}
				UserInfo targetUser = userFromName(target.toLowerCase());
				if(targetUser == null) {
		...
		if(memberRank >= LEVEL_SAFELIST) {
			...
			if(command.equals("allstaff")) {
				...
				chatthread.queueChat(garena.id, owner + " staff team currently in room:\nRoot Admins:" + rootAdmins + "\nAdmins: " + admins + "\nExaminers:" + examiners, ANNOUNCEMENT);
				return null;
				...
				String target = trimUsername(removeSpaces(payload));
				if(target.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "whois <username>. For further help use " + trigger + "help whois", member.userID);
					return null;
				}
				return whois(target);
				...
				if(target.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "whoisuid <uid>. For further help use " + trigger + "help whoisuid", member.userID);
					return null;
				}
				...
				chatthread.queueChat(garena.id, "Online since: " + startTime + ". There are " + databaseSize + " users stored in the database. " + numBanned + " players have been banned by this bot. " + numKicked + " players have been kicked by this bot. There " + muted_grammar + numMuted + " player" + muted_plural + " currently muted. There " + ann_grammar + numAnnouncement + " announcement" + ann_plural + " currently being sent.", ANNOUNCEMENT);
				return null;
			}
		}
		...
		if(memberRank >= LEVEL_PUBLIC) {
			if(command.equals("commands")) {
				...
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "voteleaver <username>. For further help use " + trigger + "help baninfo", member.userID);
					return null;
				}
				...
				payload = trimUsername(removeSpaces(payload));
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "baninfo <username>. For further help use " + trigger + "help baninfo", member.userID);
					return null;
				}
				if(sqlthread.doesBanExist(payload.toLowerCase())) {
				...
				payload = trimUsername(removeSpaces(payload));
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "kickinfo <username>. For further help use " + trigger + "help kickinfo", member.userID);
					return null;
				}
				...
				payload = removeSpaces(payload);
				if(payload.equals("")) {
					chatthread.queueChat(garena.id, "Invalid format detected. Correct format is " + trigger + "alias <command>. For further help use " + trigger + "help alias", member.userID);
					return null;
				}
	...
	public void chatReceived(GarenaInterface source, MemberInfo player, String chat, boolean whisper) {
		...
		if(bannedWordDetectType > 1 && memberRank == LEVEL_PUBLIC) {
			for(int i = 0; i < bannedWords.size(); i++) {
				if(chat.toLowerCase().indexOf(bannedWords.get(i)) > -1) {
					if(bannedWordDetectType == 1) {
						chatthread.queueChat(garena.id, "Warning! <" + player.username + "> " + banned_word_detect_message, MAIN_CHAT);
			...
			for(int i = 0; i < player.lastMessages.length; i++) { //checks current chat against previous chat
				if(chat.equals(player.lastMessages[i])) {
					if(channelAdmin && memberRank == LEVEL_PUBLIC) {
						...
						String ipAddress = player.externalIP.toString().substring(1);
						if(sqlthread.ban(player.username, ipAddress, currentDate, "Auto detection", "Bypassing flood protection", expireDate)) {
							chatthread.queueChat(garena.id, "Successfully banned <" + player.username + "> for 1 year from joining GCB. Banned by: Auto detection. Reason: bypassing flood protection. For information about this ban use " + trigger + "baninfo " + player.username, MAIN_CHAT);
							return;
						} else {
							chatthread.queueChat(garena.id, "Failed. There was an error with your database. Please inform GG.Dragon", ANNOUNCEMENT);
							return;
		...
		if(checkSpam) {
			...
			if(numNewLines > spamMaxLines) {
				...
				if(player.numWarnings > spamKick) {
					...
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked! (use less new lines). FINAL WARNING!!!", ANNOUNCEMENT);
					return;
				} else if(player.numWarnings == spamKick-1) {
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked! (use less new lines). Second warning!", ANNOUNCEMENT);
					return;
				} else if(player.numWarnings == spamKick-2) {
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked. (use less new lines). First warning!", ANNOUNCEMENT);
					return;
				}
				...
				if(numEqualitySigns > spamMaxEquality) {
					...
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked! (use less equality symbols). FINAL WARNING!!!", ANNOUNCEMENT);
					return;
				} else if(player.numWarnings == spamKick-1) {
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked! (use less equality symbols). Second warning!", ANNOUNCEMENT);
					return;
				} else if(player.numWarnings == spamKick-2) {
					chatthread.queueChat(garena.id, "<" + player.username + ">: stop spamming or you will be kicked. (use less equality symbols). First warning!", ANNOUNCEMENT);
					return;
				}
			} else {
				player.numWarnings--;
		...
		if(player != null && chat.startsWith("?trigger")) {
			...
			if(whisper) {
				chatthread.queueChat(garena.id, trigger_msg, player.userID);
			} else {
				chatthread.queueChat(garena.id, trigger_msg, MAIN_CHAT);
			}
		}
		...
		if(player != null && chat.startsWith(trigger) && !chat.substring(1).startsWith(trigger) && !chat.equals(trigger)) {
			...
			if(response != null) {
				if(whisper) {
					chatthread.queueChat(garena.id, response, player.userID);
				} else {
					chatthread.queueChat(garena.id, response, MAIN_CHAT);
				}
			}
		}
	}
	...
	public void playerJoined(GarenaInterface source, MemberInfo player) {
		...
		if(userJoinAnnouncement) {
			if(userRank == LEVEL_ROOT_ADMIN) {
				chatthread.queueChat(garena.id, "Root Administrator <" + player.username + "> has entered the room", ANNOUNCEMENT);
			} else if(userRank == LEVEL_ADMIN) {
				chatthread.queueChat(garena.id, "Administrator <" + player.username + "> has entered the room", ANNOUNCEMENT);
			} else if(userRank == LEVEL_EXAMINER) {
				chatthread.queueChat(garena.id, "Examiner <" + player.username + "> has entered the room", ANNOUNCEMENT);
			} else if(userRank == LEVEL_VIP) {
				chatthread.queueChat(garena.id, "V.I.P < " + player.username + "> has entered the room", ANNOUNCEMENT);
			}
		}
		if(publicUserMessage && userRank <= LEVEL_SAFELIST) {
			chatthread.queueChat(garena.id, welcome_message, player.userID);
		}
	}
